...
function remapHref(href: string): string {
  ...
  if (href.startsWith("#material-")) {
    ...
    return materialHash("en", id);
  }
  return href;
}

/** EN archive pages: English section anchors and date strings. */
export function localizeArchiveSessionForEn(session: ArchiveSession): ArchiveSession {
  return {
    ...session,
